...
module.exports = function (config_filename, logger) {
	...
	if (!config_filename) {
		config_filename = 'parts_local.json';
	}
	...
	var package_json = require(path.join(__dirname, '../package.json'));					//get release version of supplychain demo from package.json
	...
	// get the channel id on network for airplane parts
	...
	helper.getChaincodeId = function () {
		...
		logger.warn('No chaincode ID found in credentials file... might be okay if we haven\'t instantiated parts yet');
		return null;
	};
	...
	helper.getChaincodeVersion = function () {
		...
		logger.warn('No chaincode version found in credentials file... might be okay if we haven\'t instantiated parts yet');
		return null;
	};
	...
	// Config Getters
	// --------------------------------------------------------------------------------
	// get the part owner names
	helper.getPartUsernames = function () {
		return getPartsField('usernames');
	};

	// get the part's trading company name
	helper.getCompanyName = function () {
		return getPartsField('company');
	};

	// get the part's server port number
	helper.getPartsPort = function () {
		return getPartsField('port');
	};

	// get the status of parts previous startup
	...
	helper.getKeepAliveMs = function () {
		var sec = getPartsField('keep_alive_secs');
		if (!sec) sec = 30;									//default to 30 seconds
		return (sec * 1000);
	};

	// safely retrieve parts fields
	function getPartsField(parts_field) {
		try {
			if (helper.config[parts_field]) {
				return helper.config[parts_field];
			}
			else {
				logger.warn('"' + parts_field + '" not found in config json: ' + helper.config_path);
				return null;
			}
		}
		catch (e) {
	...
	helper.makeUniqueId = function () {
		...
		return 'parts-' + helper.getNetworkName() + '-' + channel + '-' + first_peer;
	};

	// build the parts lib module options
	helper.makePartsLibOptions = function () {
		const channel = helper.getChannelId();
	...
	// check if parts UI and parts chaincode work together
	helper.errorWithVersions = function (v) {
		...
		if (v.parsed[0] !== version[0]) {					//only check the major version
			...
			logger.warn('----------------------------- Ah! -----------------------------');
			logger.warn('---------------------------------------------------------------');
			logger.error('Looks like you are using an old version of parts chaincode...');
