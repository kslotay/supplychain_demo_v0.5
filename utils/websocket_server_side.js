...
module.exports = function (g_options, fcw, logger) {
	...
	// Setup WS Module
	//--------------------------------------------------------
	ws_server.setup = function (l_broadcast, l_parts_lib) {
		broadcast = l_broadcast;
		parts_lib = l_parts_lib;
		...
		enrollInterval = setInterval(function () {					//to avoid REQUEST_TIMEOUT errors we periodically re-enroll
			...
			fcw.enroll(enroll_options, function (err, enrollObj2) {
				if (err == null) {
					//parts_lib = require(path.join(__dirname, './parts_cc_lib.js'))(enrollObj2, opts, fcw, logger);
				}
	...
	ws_server.process_msg = function (ws, data) {
		...
		if (parts_lib === null) {
			logger.error('parts lib is null...');				//can't run in this state
			return;
		}

		// create a new part
		if (data.type === 'create') {
			logger.info('[ws] create parts req');
			options.args = {
				color: data.color,
				size: data.size,
				part_owner: data.username,
				owners_company: data.company,
				owner_id: data.owner_id,
				auth_company: process.env.part_company,
			};

			parts_lib.create_a_part(options, function (err, resp) {
		...
		// transfer a part
		else if (data.type === 'transfer_part') {
			...
			parts_lib.set_part_owner(options, function (err, resp) {
				if (err != null) send_err(err, data);
				else options.ws.send(JSON.stringify({ msg: 'tx_step', state: 'finished' }));
			});
		}
		...
		// delete part
		else if (data.type === 'delete_part') {
			logger.info('[ws] delete part req');
			options.args = {
				part_id: data.id,
				auth_company: process.env.part_company
			};

			parts_lib.delete_part(options, function (err, resp) {
				if (err != null) send_err(err, data);
				else options.ws.send(JSON.stringify({ msg: 'tx_step', state: 'finished' }));
			});
		}

		// get all owners, parts, & companies
		...
		// get history of part
			...
			if (data.part_id) {
				...
				parts_lib.get_history(options, function (err, resp) {
		...
		// disable part owner
			...
			if (data.owner_id) {
				...
				parts_lib.disable_owner(options, function (err, resp) {
					if (err != null) send_err(err, resp);
					else options.ws.send(JSON.stringify({ msg: 'tx_step', state: 'finished' }));
				});
			}
	...
	ws_server.check_for_updates = function (ws_client) {
		parts_lib.channel_stats(null, function (err, resp) {
			var newBlock = false;
			if (err != null) {
				var eObj = {
					msg: 'error',
	...
	// read complete state of part world
	function read_everything(ws_client, cb) {
		...
		parts_lib.read_everything(options, function (err, resp) {
			if (err != null) {
				console.log('');
				...
				if (data && data.owners && data.parts) {
					console.log('');
					logger.debug('[checking] number of owners:', data.owners.length);
					logger.debug('[checking] number of parts:', data.parts.length);
				}

				data.owners = organize_usernames(data.owners);
				data.parts = organize_parts(data.parts);
	...
	function organize_usernames(data) {
		...
		for (var i in data) {						//lets reformat it a bit, only need 1 peer's response
			...
			if (temp.company === process.env.part_company) {
	...
	//
	function organize_parts(allParts) {
		var ret = {};
		for (var i in allParts) {
			if (!ret[allParts[i].owner.username]) {
				ret[allParts[i].owner.username] = {
					owner_id: allParts[i].owner.id,
					username: allParts[i].owner.username,
					company: allParts[i].owner.company,
					parts: []
				};
			}
			ret[allParts[i].owner.username].parts.push(allParts[i]);
		}
		return ret;
	}
