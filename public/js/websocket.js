...
/* global show_start_up_step, build_notification, build_user_panels, build_company_panel, populate_users_parts, show_tx_step*/
/* global getRandomInt, block_ui_delay:true, build_a_tx, auditingPart*/
/* exported transfer_part, record_company, connect_to_server, refreshHomePanel, pendingTxDrawing*/
...
function connect_to_server() {
	...
	function onOpen(evt) {
		...
		addshow_notification(build_notification(false, 'Connected to Parts application'), false);
		connected = true;
	}
	...
	function onClose(evt) {
		...
		connected = false;
		addshow_notification(build_notification(true, 'Lost connection to Parts application'), true);
	...
	function onMessage(msg) {
		try {
			...
			if (msgObj.msg === 'everything') {
				...
				for (var i in msgObj.everything.parts) {
					populate_users_parts(msgObj.everything.parts[i]);
				}

				start_up = false;
				$('.partsWrap').each(function () {
					if ($(this).find('.innerPartWrap').find('.ball').length === 0) {
						$(this).find('.noPartsMsg').show();
					}
				});
			}

			//parts
			else if (msgObj.msg === 'users_parts') {
				console.log(wsTxt + ' rec', msgObj.msg, msgObj);
				populate_users_parts(msgObj);
			}
			...
			//part owners
				...
				console.log(wsTxt + ' sending get_parts msg');
			}
			...
			//all parts sent
			else if (msgObj.msg === 'all_parts_sent') {
				console.log(wsTxt + ' rec', msgObj.msg, msgObj);
				start_up = false;

				$('.partsWrap').each(function () {
					console.log('checking', $(this).attr('owner_id'), $(this).find('.innerPartWrap').find('.ball').length);
					if ($(this).find('.innerPartWrap').find('.ball').length === 0) {
						$(this).find('.noPartsMsg').show();
					}
				});
...
//transfer_part selected ball to user
function transfer_part(partId, to_owner_id) {
	show_tx_step({ state: 'building_proposal' }, function () {
		...
		console.log(wsTxt + ' sending transfer part msg', obj);
		ws.send(JSON.stringify(obj));
		refreshHomePanel();
	});
}
