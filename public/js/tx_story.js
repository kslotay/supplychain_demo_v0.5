...
function show_tx_step(obj, cb_orig){
	...
	setTimeout(function(){													//wait for initial panel fade in
		...
		//error states
			...
			$('#endorsePartStable .fa-check').removeClass('fa-check').addClass('fa-close');
			$('#endorsePartStable span').fadeIn();
			$('.closeTxStory').show();
		}
...
//1. animate borders to join part in center
function story1_animation(cb){
	var dist = 50;
	$('#partBorderTop, #partBorderBottom, #partBorderLeft, #partBorderRight').show();
	$('#partBorderTop').animate({top: '+=' + dist}, {
		duration: 1800,
		complete: cb
	});
	$('#partBorderBottom').animate({top: '-=' + dist}, {duration: 1300});
	$('#partBorderLeft').animate({left: '+=' + dist}, {duration: 800});
	$('#partBorderRight').animate({left: '-=' + dist}, {duration: 800});
}

//1. show part that will roll
//2. roll it
//3. hide rolled part
//4. show endorse part with (icon hidden)
function story2_animation(cb){
	$('#proposePart').show();
	$('#proposePartStable').removeClass('hideBorders');
	...
	roll_ball('#proposePart', diff, function(){
		$('#proposePart').hide();
		$('#endorsePartStable').show();
		if(cb) cb();
	});
}

//1. show the part that will roll
//2. roll endorsed part
//3. show orderer parts
//4. hide rolled part
//5. build box around parts
function story3_animation(cb){
	...
	roll_ball('#endorsePart', diff, function(){
		$('#endorsePartStable').show();
		$('.ordererParts').fadeIn();
		setTimeout(function(){
...
//1. fade in solid box around parts
//2. animate it right
