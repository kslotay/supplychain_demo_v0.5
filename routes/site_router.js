...
function build_bag(req) {
	return {
		...
		part_company: process.env.part_company,
...
function get_credential_data() {
	...
	var ret = {
		...
		part_owners: helper.getPartUsernames(),
	};
	for (var i in ret) {
...
router.route('/login').get(function (req, res) {
	res.render('login', { title: 'Parts - Login', bag: build_bag(req) });
});
...
function route_me(req, res) {
	if (!req.session.user || !req.session.user.username) {
		...
		res.render('parts', { title: 'Parts - Home', bag: build_bag(req) });
	}
}
